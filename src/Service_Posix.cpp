        ...
        )
{
    ...
    if( pid_file_name.length()>0 )
    {
        ...
        if( pid_fd>=0 )
        {
            ...
            {
                ob_log_error( "%s: Unable to lock pid file: %s", identity.c_str(), pid_file_name.c_str() );
                abort();
            }
            sprintf( tmpbuf, "%ld\n", (long)getpid() );
            ...
            {
                ob_log_error( "%s: Error writing pid file: %s", identity.c_str(), pid_file_name.c_str() );
                abort();
            }
            atexit( daemon_end );
        }
        ...
        {
            ob_log_error( "%s: Error creating pid file: %s", identity.c_str(), pid_file_name.c_str() );
            abort();
        }
    }
    prepare_child_start();
