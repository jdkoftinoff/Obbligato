...
void Operation::dump( std::ostream &os ) const
{
    ...
    os << label_fmt( "operation_description" ) << stringblock_fmt( operation_description() ) << std::endl;
    os << label_fmt( "progress_in_permil" ) << operation_progress_in_permil() << std::endl;
    os << label_fmt( "operation_id" ) << "( " << m_operation_id.first << "," << m_operation_id.second << " )" << std::endl;
    os << label_fmt( "primary_target" ) << m_primary_target.get() << std::endl;
    os << label_fmt( "current_sub_operation" ) << m_current_sub_operation.get();
    if ( m_current_sub_operation )
    {
        os << "( " << m_current_sub_operation->operation_id().first << "," << m_current_sub_operation->operation_id().second
           << " )"
           << ":" << m_current_sub_operation->operation_description() << std::endl;
    }
    os << std::endl;
...
void Operation::notify_targets_operation_in_progress( int permil_done )
{
    if ( m_primary_target )
    {
        m_primary_target->requested_operation_in_progress( m_operation_id, permil_done );
    }
    for ( NotificationTargetPtrVector::iterator i = m_targets.begin(); i != m_targets.end(); ++i )
    {
        NotificationTarget *t = i->get();
