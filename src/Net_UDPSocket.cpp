...
UDPSocket::UDPSocket( shared_ptr<Pool<Packet>> &pool, Address local_addr, Address default_dest_addr )
    : m_local_addr( local_addr ), m_default_dest_addr( default_dest_addr ), m_pool( pool )
{
    ...
    do
    {
        m_fd = ::socket( m_local_addr.get_sa_family(), SOCK_DGRAM, 0 );
...
void UDPSocket::send( PacketPtr const &pkt )
{
    ...
    do
    {
        r = ::sendto( m_fd, (const char *)pkt->payload().data.data(), pkt->payload().data.size(), 0, dest_addr, dest_addr_len );
...
#endif
        r = ::recvfrom( m_fd, d, pkt->payload().data.size(), 0, (struct sockaddr *)&addr, &addr_len );
    } while ( r < 0 && ( errno == EINTR ) );
...
bool UDPSocket::join_multicast( const char *interface_name, Address const &mcast_addr )
{
    bool r = false;
    ...
    if ( mcast_addr.get_sa_family() == AF_INET6 )
    {
        ...
        if ( ::setsockopt( m_fd, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *)&multicast_request, sizeof( multicast_request ) ) == 0 )
        {
            r = true;
        }
    }
    else
    {
        if ( mcast_addr.get_sa_family() == AF_INET )
        {
            ...
            if ( ::setsockopt( m_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&multicast_request, sizeof( multicast_request ) )
                 == 0 )
            {
                r = true;
            }
