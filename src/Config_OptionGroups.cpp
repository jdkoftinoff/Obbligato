    ...
    while (fgets(bline, 255, fp) != NULL) {
        ...
        if (line.length() > 1) // \n is part of the string
            ...
            if ((last_non_blank - first_non_blank > 0) &&
                line[first_non_blank] != '#') // skip blank and comment lines
                {
                size_t end_of_key =
                    line.find_first_of("\r\n\t =", first_non_blank);
                std::string key =
                    line.substr(first_non_blank, end_of_key - first_non_blank);
                ...
                if (first_value_quote_pos == line.npos) {
                    ...
                    value = line.substr(first_value_quote_pos + 1,
                                        last_quote - first_value_quote_pos - 1);
                }
                ...
                try {
                    ...
                    if (fail_on_invalid_args) {
                        fclose(fp);
                        throw std::invalid_argument(form<4096>(
                            "File '%s':%d - %s", fname, line_count, e.what()));
                    }
                }
            }
        }
...
bool OptionGroups::parse(char const **argv, std::string const &banner,
                         std::string const &version, std::ostream &os) {
    if (!m_finalized) {
        finalize();
    }
    ...
    try {
        while (*argv) {
            if ((*argv)[0] == '-' && (*argv)[1] == '-') {
                ...
                size_t name_len = (pos_equals != 0 ? (pos_equals - pos_name)
                                                   : strlen(pos_name));
...
bool OptionGroups::parse(char const **argv, char const *version) {
    ...
    try {
        if (!parse(argv + 1, std::string(argv[0]), std::string(version),
                   std::cout)) {
            return false;
        }
    }
...
void OptionGroups::dump_with_description(std::ostream &os,
