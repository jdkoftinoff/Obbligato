...
class AtomicInt {
  public:
    ...
    bool compare_and_set(type old_value, type new_value) {
        return ::Obbligato::Atomic::compare_and_set(&value, old_value,
                                                    new_value);
    }
...
class AtomicInt {
  public:
    ...
    explicit AtomicInt(type v = 0) : value(v) {}

    operator type(void) const { return __sync_fetch_and_add(&value, 0); }
    ...
    int32_t operator+=(type v) { return __sync_add_and_fetch(&value, v); }

    int32_t operator-=(type v) { return __sync_sub_and_fetch(&value, v); }

    /* prefix operator */
    type operator++(void) { return __sync_add_and_fetch(&value, 1); }

    /* prefix operator */
    type operator--(void) { return __sync_sub_and_fetch(&value, 1); }

    /* postfix operator */
    type operator++(int) { return __sync_fetch_and_add(&value, 1); }

    /* postfix operator */
    type operator--(int) { return __sync_fetch_and_sub(&value, 1); }
...
class AtomicInt {
  public:
    ...
    /* prefix operator */
    type operator++(void) { return operator+=(1); }

    /* prefix operator */
    type operator--(void) { return operator-=(1); }

    /* postfix operator */
    type operator++(int) { return __gnu_cxx::__exchange_and_add(&value, 1); }
...
class AtomicIncFunctor {
  public:
    AtomicInt::type operator()(const AtomicInt::type &v) const { return v + 1; }
};
...
class AtomicXorFunctor {
  ...
  public:
    AtomicXorFunctor(AtomicInt::type x) : m_xor(x) {}
...
class AtomicIncMaskFunctor {
  ...
  public:
    ...
    AtomicIncMaskFunctor(AtomicInt::type mask) : m_mask(mask) {}
...
class AtomicIncModFunctor {
  ...
  public:
    ...
    AtomicIncModFunctor(AtomicInt::type mod) : m_mod(mod) {}
