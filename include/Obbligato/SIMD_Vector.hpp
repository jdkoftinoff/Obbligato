...
template <typename T>
inline std::complex<T> &splat(std::complex<T> &v, std::complex<T> const &a) {
    v = a;
    return v;
}
...
template <typename T> inline std::complex<T> &zero(std::complex<T> &v) {
...
template <typename T> inline std::complex<T> &one(std::complex<T> &v) {
...
class OBBLIGATO_PLATFORM_VECTOR_ALIGN SIMD_Vector {
  public:
    ...
    SIMD_Vector(std::initializer_list<value_type> list) {
        size_t n = 0;
        for (auto v = std::begin(list); v != std::end(list) && n < vector_size;
             ++v) {
            m_item[n++] = *v;
        }
    }
...
template <typename T, size_t N> class SIMD_VectorRef {
  public:
    ...
    template <typename CharT, typename TraitsT>
    friend std::basic_ostream<CharT, TraitsT> &operator<<(
        std::basic_ostream<CharT, TraitsT> &str, simd_ref_type const &a) {
        str << a.m_ref;
        return str;
    }
    ...
    friend simd_type operator+=(simd_ref_type &a, simd_ref_type const &b) {
        a.m_ref += b.m_ref;
        return a.m_ref;
    }
    ...
    friend simd_type operator+(simd_ref_type const &a, value_type const &b) {
        simd_type r;
        r = a.m_ref + b;
        return r;
    }
    ...
    friend simd_type operator+(simd_ref_type const &a, simd_ref_type const &b) {
        simd_type r;
        r = a.m_ref + b.m_ref;
        return r;
    }
    ...
    friend simd_type operator+(simd_ref_type const &a, simd_type const &b) {
        simd_type r;
        r = a.m_ref + b;
        return r;
    }
    ...
    friend simd_type operator+(simd_type const &a, simd_ref_type const &b) {
        simd_type r;
        r = a + b.m_ref;
        return r;
    }
...
template <typename T, size_t N> class SIMD_VectorConstRef {
  public:
...
template <typename T> struct is_simd_ref : std::false_type {};
...
template <typename T> struct is_simd_const : std::false_type {};
...
template <typename T> struct simd_value_type {
    typedef T type;
};

template <typename T, size_t N> struct simd_value_type<SIMD_Vector<T, N>> {
    typedef T type;
};

template <typename T, size_t N> struct simd_value_type<SIMD_VectorRef<T, N>> {
    typedef T type;
};
...
template <typename T, size_t X>
struct simd_size<SIMD_VectorRef<T, X>> : public std::integral_constant<size_t,
                                                                       X> {};

template <typename T, size_t X>
struct simd_size<SIMD_VectorConstRef<T, X>> : public std::integral_constant<
    size_t, X> {};
...
template <typename T, size_t X>
struct simd_flattened_size<SIMD_Vector<T, X>> : public std::integral_constant<
    size_t, X> {};

template <typename T, size_t X, size_t Y>
struct simd_flattened_size<SIMD_Vector<
    SIMD_Vector<T, Y>, X>> : public std::integral_constant<size_t, X *Y> {};

template <typename T, size_t X, size_t Y, size_t Z>
struct simd_flattened_size<
    SIMD_Vector<SIMD_Vector<SIMD_Vector<T, Z>, Y>,
                X>> : public std::integral_constant<size_t, X *Y *Z> {};

template <typename T, size_t X, size_t Y, size_t Z, size_t W>
struct simd_flattened_size<
    SIMD_Vector<SIMD_Vector<SIMD_Vector<SIMD_Vector<T, W>, Z>, Y>,
                X>> : public std::integral_constant<size_t, X *Y *Z *W> {};

template <typename T> struct simd_flattened_type {
    typedef T type;
};

template <typename T, size_t X> struct simd_flattened_type<SIMD_Vector<T, X>> {
    typedef T type;
};
...
struct simd_flattened_type<
    SIMD_Vector<SIMD_Vector<SIMD_Vector<SIMD_Vector<T, W>, Z>, Y>, X>> {
    typedef T type;
};

/**@}*/

template <typename SimdT, typename std::enable_if<!is_simd_ref<SimdT>::value,
                                                  bool>::type sfinae = true>
SIMD_VectorRef<typename SimdT::value_type, SimdT::vector_size>
make_simd_ref(SimdT &other) {
    return SIMD_VectorRef<typename SimdT::value_type, SimdT::vector_size>(
        other);
}
...
make_simd_ref(SimdT const &other) {
    return SIMD_VectorConstRef<typename SimdT::value_type, SimdT::vector_size>(
        other);
}

template <typename SimdT, typename std::enable_if<is_simd_ref<SimdT>::value,
                                                  bool>::type sfinae1 = true,
          typename std::enable_if<!is_simd_const<SimdT>::value, bool>::type
              sfinae2 = true>
SIMD_VectorRef<typename SimdT::value_type, SimdT::vector_size>
make_simd_ref(SimdT &other) {
    return SIMD_VectorRef<typename SimdT::value_type, SimdT::vector_size>(
...
typename std::enable_if<!is_simd<T>::value, T>::type &apply(T &r, Func f,
                                                            Args &&... args) {
    r = f(args...);
    return r;
}
...
template <typename T>
inline std::complex<T> &set_item(std::complex<T> &v, std::complex<T> const &a,
                                 size_t x = 0) {
    (void)x;
    v = a;
    return v;
...
inline double &set_flattened_item(double &v, double const &a, size_t i) {
    (void)i;
    v = a;
    return v;
}
...
std::complex<T> &set_flattened_item(std::complex<T> &v,
                                    std::complex<T> const &a, size_t i) {
    (void)i;
    v = a;
    return v;
}
...
SimdT &set_flattened_item(SimdT &v,
