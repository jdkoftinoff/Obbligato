...
class Choice
{
    ...
    template<typename T>
    using none = typename std::enable_if<!std::is_same<Choice, T>::value>::type;
...
public:
    ...
    Choice(T &&x) : p{new data<U>{std::forward<T>(x)}} { }

    Choice &operator=(Choice s) { swap(*this, s); return *this; }
