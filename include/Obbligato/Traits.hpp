...
template <typename T> struct IsMathVector : public FalseTrait<T> {};
...
template <size_t SZ> struct SizeToStorage {
    typedef void storage_type;
};

template <> struct SizeToStorage<1> {
    typedef IEEE::Octet storage_type;
};

template <> struct SizeToStorage<2> {
    typedef IEEE::Doublet storage_type;
};

template <> struct SizeToStorage<4> {
    typedef IEEE::Quadlet storage_type;
};

template <> struct SizeToStorage<8> {
...
template <> struct Storage<IEEE::Octet> {
...
template <> struct Storage<IEEE::Doublet> {
...
template <> struct Storage<IEEE::Quadlet> {
...
template <> struct Storage<IEEE::Octlet> {
