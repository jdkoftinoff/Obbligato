...
class OptionBase {
  ...
  public:
    OptionBase(std::string prefix_, std::string key_,
               std::string default_value_, std::string description_)
        : m_prefixed_key(prefix_ + std::string(".") + key_),
          m_default_value(default_value_), m_description(description_),
          m_next(nullptr) {}
    ...
    virtual void help(std::ostream &os) const {
        os << form<1024>("    %-32s: %s (%s)", m_prefixed_key.c_str(),
                         m_description.c_str(), m_default_value.c_str())
           << std::endl;
    }
    ...
    std::string const &prefixed_key() const { return m_prefixed_key; }

    std::string const &default_value() const { return m_default_value; }
