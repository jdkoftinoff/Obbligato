...
class LinkStatusWin32 : public LinkStatusBase {
  public:
    ...
    LinkStatusWin32() {
        get_interfaces();
        NotifyIpInterfaceChange(AF_UNSPEC, interface_change_callback, this,
                                true, &m_notification_handle);
    }

    ~LinkStatusWin32() { CancelMibChangeNotify2(m_notification_handle); }

    bool get_link_status(const char *eth) {
        std::string name(eth);
        for (Interfaces::const_iterator it = m_last_state.begin();
             it != m_last_state.end(); it++) {
  ...
  private:
        ...
        if (GetIfTable2(&table) == NO_ERROR) {
            for (ULONG i = 0; i < table->NumEntries; ++i) {
                if (table->Table[i]
                    ...
                    WideCharToMultiByte(CP_ACP, 0, table->Table[i].Alias, -1,
                                        temp, IF_MAX_STRING_SIZE, NULL, NULL);
                    li.alias = std::string(temp);

                    StringFromCLSID(table->Table[i].InterfaceGuid, &guid);
                    WideCharToMultiByte(CP_ACP, 0, guid, -1, temp,
                                        IF_MAX_STRING_SIZE, NULL, NULL);
                    li.guid = std::string(temp);
                    CoTaskMemFree(guid);
                    li.connected = (table->Table[i].MediaConnectState ==
    ...
    static void WINAPI
    interface_change_callback(PVOID caller_context, PMIB_IPINTERFACE_ROW row,
        ...
        if (notification_type == MibParameterNotification) {
            LinkStatusWin32::Interfaces::iterator it =
                pthis->m_last_state.find(row->InterfaceLuid.Value);
            if (it != pthis->m_last_state.end()) {
                ...
                if (GetIfEntry2(&if_row) == NO_ERROR) {
                    if ((*it).second.connected != (if_row.MediaConnectState ==
                                                   MediaConnectStateConnected))
                        (*it).second.did_change = true;

                    (*it).second.connected = (if_row.MediaConnectState ==
                                              MediaConnectStateConnected);
                }
            }
        }
